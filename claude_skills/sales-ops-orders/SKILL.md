...
`order_customer.customer_type` classifies `mapped_cust_id` as `person`, `kiosk`, `internal`, or `aggregator` (NULL when unidentified). **38% of identified orders belong to non-person ids** — shared outdoor-kiosk terminal accounts, employee/developer accounts, and one orphan third-party aggregator id (`19192`, which absorbs 2.48M lifetime orders across 89 stores and 30,039 distinct `@itsacheckmate.com` emails).
...
- **Sales / order / channel metrics** → do **not** filter. Those are real orders and real revenue; excluding them understates sales by ~$3.8M/month.
...
- **Employee/test exclusion:** internal accounts are now identifiable via `customer_type = 'internal'` (`@cafezupas.com`, `@tkxel.com`, `@tkxel.io` in `pulse.customers.email`) — 108 ids / 894 orders in June 2026. This replaces the old `mapped_domain` mart gap for identified orders; unidentified internal orders still can't be flagged.
