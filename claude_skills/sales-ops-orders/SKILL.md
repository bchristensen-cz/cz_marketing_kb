...
- **⚠️ Demographics are the exception to the line above: `gender`, `birthday` and `age` landed on `sales_ops.customer_attribute` on 2026-09-15 and did NOT reach `claude.order_customer`** (verified against the live view 2026-09-16 — all three absent from `claude.INFORMATION_SCHEMA.COLUMNS`, because the view folds the attribute table in through a select list, not `select *`). A demographic question from a standard user errors with `Unrecognized name`, so say the columns exist upstream and are **pending a steward call on exposure** — do not improvise an age from another table. Coverage measured 2026-09-16 (1,335,690 customers): `gender` 83.84% populated and lowercase two-valued (`female` 59.63% / `male` 24.21% / NULL 16.16%, no `other` sentinel — quote shares on the populated base); `birthday` 20.18% with an unvalidated 1877→2022 range; `age` only 9.03%, and **149,026 customers have a birthday but a NULL age**, so `age` is not a `date_diff` off `birthday` and the two are not interchangeable. 6 ages under 13 and 27 over 100 survive into the column.
