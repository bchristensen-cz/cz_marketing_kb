...
Same as `sales-ops-orders` — read that section for the full rules and a worked example. In short: fully qualified table names with **backticks around the project only** (`` `marketing-data-442316`.claude.loyalty_user lu ``, never the whole path) **and** an alias on every single column reference (even in single-table queries); lowercase except where a schema name or compared literal needs case; leading commas each followed by one space, one column per line with **the first field flush with `select` rather than indented** and **`from` / `group by` / `order by` values kept on the keyword line**, **exactly one space before `as` (no alignment padding, and none inside a `case` either)**, and **a `case` broken across lines with its `when` / `else` branches indented one tab once it has 2+ `when`s**; `where 1=1` first, then one `and` per line; each join with `on` lined up beneath it and one extra indent per successive join — joins and multi-branch `case` branches are the only indented lines in a query. Fixed aliases: `order_customer` → `oc`, `order_lines` → `ol`.
...
| Users missing from `sessionM.users` | **688 `user_id`s / 5,011 `user_offers` rows have activity but no row in `sessionM.users`** (measured 2026-08-27, `create_date < current_date() -1`; 0.045% of 1,518,699 offer users). **Verified against the GCS source files, not inferred:** they are absent from the `users` extract itself. Every one of the 30,155 distinct ids in the 2026-08-17→08-27 dumps is present in `sessionM.users` — **0 load drops** — and all 688 orphans are absent from those same dumps, and from the 27 daily dumps covering 2026-05-20→06-15. They are real members: **523** have `tier_member_history` rows, **500** have `user_point_accounts` and `user_point_transactions`, **0** appear in `privacy_requests` (so not deletions) and **0** in `external_user_mappings` (so no `sm_external_user_id`). Concentrated in 2026-07 (**367 of 688** by first offer month). Consequence: `claude.loyalty_offer_usage` left-joins `loyalty_user`, so their offer rows survive with **NULL identity** — any inner join, email filter or `member_program` filter drops them silently. Supersedes the earlier "9 catering tier rows with no user" note, which was the same defect seen through one tier |
| The `users` extract is delta-only — nothing self-heals | Each daily dump in `gs://sessionm/prod_datadumps/<date>/users/` is **12–18k rows** — a rolling ~7–9 day `updated_at` window — not a 1.8M-row snapshot, and **no full snapshot exists anywhere in the bucket** (the oldest dump, 2023-03-08, is 211 bytes). `sessionM.users` (1,796,806 rows, all distinct, `created_at` from 2023-05-08) is therefore an accumulation of deltas, so **a user the upstream extract skips once is missing permanently** and cannot be repaired from GCS — it needs a re-export from SessionM. Whole days can also be absent: **no dump at all on 2026-05-31**, and no ETL run on 2026-08-22 (visible as a gap in the `etl_time` values carried inside later files) |
