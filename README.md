...
4. Write ALL SQL in the steward's format (see "SQL style" in `claude_skills/sales-ops-orders/SKILL.md`): fully qualified table names with backticks around the project only (`` `marketing-data-442316`.sales_ops.order_customer oc ``) **and a table alias on every column reference**, lowercase whenever possible, fixed aliases (`oc` = `order_customer`, `ol` = `order_lines`), leading commas with the **first field flush with `select` / `group by` / `order by` (never indented)** and **one space before `as` (never alignment-padded, and no padding inside `case` expressions)**, `where 1=1` + one `and` per line, joins with `on` lined up beneath them and one extra indent per successive join — a join and its `on` are the **only** indented lines in a query. The steward diagnoses user-generated SQL — it must be instantly readable in his layout.
