...
-- (see the gotcha in the dictionary, Asana 1217684772713570).
...
, safe_cast(s.store_phone as int64)
, s.is_comp_store
...
AND t.is_comp_store <> s.is_comp_store
;
...
--    2026-08-20 across 98 stores). Split-timezone states are deliberately ABSENT from
--    this map so a store there stays NULL and trips the assert below, rather than being
--    guessed an hour wrong: north Idaho is Pacific, El Paso is Mountain, and Oregon,
--    the Dakotas, Kansas, Nebraska, Florida, Michigan, Indiana, Kentucky and Tennessee
--    are all split. A wrong-but-populated timezone is worse than a NULL, because
--    order_timestamp_utc then looks fine and is silently off by an hour.
UPDATE `marketing-data-442316`.sales_ops.store_info t
SET
...
--
--    Verified 2026-08-20: this reproduces 87 of 87 existing assignments once cluster 12
...
--    the placeholder instead of the Chicago/Milwaukee Corridor.
--
...
--    latitude / longitude cannot be derived in BigQuery - staging carries no coordinate
--    and the ZIP centroid is up to 70 miles off - so that one is a prompt for a human
--    geocode, not something a later statement can fix.
...
-- 6c) A trading store must have coordinates. Requires a human geocode of
--     store_address / store_city / store_state / store_zip - see the dictionary for
--     the method and the confidence check (an exact Google Maps /maps/place/ match
--     versus a /maps/search/ fallback).
assert (
  select count(*) = 0
  from `marketing-data-442316`.sales_ops.store_info s
  where 1=1
