...
Both have been validated against BigQuery (dry run, zero errors). Use them as the starting point rather than rewriting the unions by hand — that's where errors creep in. Full column docs for all 69 tables: `data_dictionaries/braze_data_dictionary.md`.
...
| Content Card (banner) | `contentcard_send` | `contentcard_click` |
| In-app message | `inappmessage_impression` * | `inappmessage_click` |

\* In-app messages have **no send event** in Braze Currents. The closest "the campaign was shown" signal is the **impression**, so the activity template tags it `activity_type = 'impression'` while everything else is `'send'`.
...
- **`is_canvas`** — `1` if the message originated from a Canvas, else `0`. Use it to label the source.
...
- **`send_id`** — groups all messages from one send; useful for send-level analytics. Note `sms_shortlinkclick` and `sms_inboundreceive` have **no `send_id`** (the templates null it).
...
It unions the five activity tables into a CTE `activity`, then exposes a normalized row per event with `program_id` / `program_name` / `channel` / `activity_type`. Build the "by day" answer on top:
...
| `data_dictionaries/braze_data_dictionary.md` | Full table & column dictionary for the whole `braze` dataset (69 tables). |

## When done

If you learned something new about the Braze tables during the session (new gotcha, new canonical definition, data quality issue), capture it so the next session benefits:
...
2. Add a **new file** under `learnings/` at the repo root, named `YYYY-MM-DD-short-slug.md` (format in `LEARNINGS.md`). One file per finding — never append to a shared file or rewrite this SKILL.md/dictionary mid-session; that's what keeps multi-contributor merges conflict-free.
3. A periodic consolidation pass merges vetted entries into the skill and dictionary, pushes to GitHub, and repackages/reinstalls the skill so everyone gets the update.
