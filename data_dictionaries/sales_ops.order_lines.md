...
| `item_name` | STRING | **Normalized item group name** — size prefix (REG/LG/HALF/…) stripped, leading `.`/`--` cleaned. Use this for menu-mix. Falls back to `description` when the item isn't in the item master — and those fallback rows keep any size word in the name while `item_size` reads `'Regular'` (e.g. `Mini Chocolate Chips`, which is a Mini). ⚠️ **Not a product key.** Measured 2026-08-27 over the trailing 30 days (sellable lines, stores 1111/999 excluded), **140 of 373 names span more than one `item_id`** and those names carry **50.1% of all units**: 49 split by `item_size`, 91 split by something else. `Chocolate Strawberry Cup` = two ids at $9.00 and $14.00; `Dubai Cup` = $12.00 and $18.00; `Kids Combo` = a $0.00 `Kids` bundle slot and a $7.27 `Regular` paid combo. Resolve to `item_id` before measuring, and show the average unit price in the discovery query so a wrong pick is visible. On `discount` lines (2026-08-12) this is the **discount program name** from `brink.brinkDiscounts` (`SessionM Loyalty`, `Online Discount`, …) — group on it for discount-program reporting. |
| `item_size` | STRING | Parsed size prefix: `Regular`, `Half`, `Large`, `Kids`, `Mini`, `Party`, `Tray`, `Quart`, `Medium`. **No NULLs since the 2026-08-27 rebuild** — the build coalesces the unparsed case to `'Regular'` (verified zero NULLs 2018-2026). **Stripped out of `item_name` by the build** (regex `^(REG\|Mini\|LG\|PRTY\|HALF\|Kids\|LARGE\|Medium\|Tray\|QUART) `, case-sensitive, applied once), so the spoken name and the column disagree: `item_name = 'Mini Chocolate Strawberry Cup'` returns **zero rows**; the product is `item_name = 'Chocolate Strawberry Cup' and item_size = 'Mini'`. 🚨 **`'Regular'` is a mixed bucket — it is NOT 'the regular size'.** It labels both the genuine `REG`-prefixed size and every line the build parsed no size for. Sellable lines 2026-07-29 -> 2026-08-26, stores 1111/999 excluded (measured 2026-08-27): **`Regular` = 4,019,004 of 5,269,076 lines (76.3%), of which only 192,622 carry a real `REG ` prefix** — so `item_size = 'Regular'` returns **~21x** what it returned before 2026-08-27, and any query or saved report written earlier that filters on it silently changed meaning. It is stamped on lines with no size at all: **100% of `discount` (55,667), `tip` (51,987), `fee` (28,568), `promotion` (3,130) and `gift_card` (882) lines**, plus 81.5% of `modifier` and 72.2% of `item` lines. **To recover the genuine menu size, read `description`** — it still carries the raw prefix: `regexp_contains(description, r'^REG ')`. Never present `Regular` as a size in a breakout without saying what it contains; unqualified it reads as *three-quarters of what we sell is regular-sized*, which is false. **Required as a dimension on any item-specific question** — 49 names / 21.0% of units span more than one size, and a name-only answer blends price points (Mini cup $9.00 vs Regular $14.00 -> a $10.23 average that is not a real price). Other line counts in that window: `Half` 683,604 / `Kids` 296,243 / `Large` 234,690 / `Mini` 30,892 (only `Chocolate Strawberry Cup` and `Dubai Cup`) / `Party` 4,513 / `Tray` 130; `Quart` and `Medium` **zero rows in the window** despite being in the build's CASE. |
