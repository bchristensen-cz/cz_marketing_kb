...
- **`external_user_id` is STRING; `order_customer.mapped_cust_id` is INT64** — joining them raw fails. Cast the Braze side: `cast(ce.external_user_id as int64) = oc.mapped_cust_id`. Guard against non-numeric ids with `safe_cast` if the workspace has any. (Steward pattern, 2026-07-25/26.)
