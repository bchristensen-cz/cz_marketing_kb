...
1. **An empty partition satisfies a `between` predicate.** A missing date range is the one data-quality failure that produces neither an error nor a suspicious number — it produces a *smaller* number, which reads as a real decline. **A freshness check is part of answering, not part of maintenance:** `select max(business_date)` on every fact table an answer touches, and compare them to each other. The comparison is what caught this — `order_lines` alone looks fine. **Write the check with a partition filter, one table per statement:** `select max(business_date) from claude.order_lines where business_date >= date_sub(current_date('America/Denver'), interval 10 day)` (0.03 GiB), then the same for `order_customer`. A first-session user on 2026-09-21 generated the check as `from claude.order_lines ol full outer join (select 1 as x) dummy on false, claude.order_customer oc` with no `where` at all, which is a full-history cross join of both marts; it was cancelled before billing and rewritten correctly a minute later, but a freshness check that costs more than the answer is the wrong shape.
...
1. **✅ The rewrite happened, and it adopted every prescription in this skill at once.** The 16 jobs that succeeded on 08-24 are not a partial migration — they are the canonical pattern verbatim:
   ...
   >
   > **Day 17 (Thu 09-17): quiet — 2 non-steward queries / 3.77 GiB, both `wbeaman@`, zero failures, no wall reads.** Steward: 16 unlabeled / 0.19 GiB (QuickSight dataset refreshes + the 05:30 weather-automation health checks) and 60 MCP / 3.56 GiB (the customer_attribute App User deploy and the Braze channel-value work, both already committed 09-17/18); nothing to mine. **(a) The two analyst queries are one question — "who are our $1,000+/year customers and how do they order" — and both re-derive things the marts already hold.** Query 1 (09:35, 1.54 GiB) summed `net_sales` per `mapped_cust_id` over `business_date between max(business_date) - 364 and max(business_date)`, `customer_type = 'person'`, `not is_catering`, and banded customers into $1k / $2k / $3k / $5k tiers with `count(distinct business_date)` as visit days. **`sales_ops.customer_attribute.net_sales_l365` / `orders_l365` are that number, precomputed daily, person-only, store-1111-excluded** — the tier query is a `case` over a 1.4M-row dimension instead of a 1.5 GiB fact scan. Two honest differences to state, not reasons to re-derive: the window is 365 days to yesterday rather than 364 to `max(business_date)`, and **the `_l365` columns include catering with no in-window catering net-out** (`lifetime_catering_order_count` is lifetime only). I first wrote "the residual is small" because catering and individual customers are separate identity populations; **that was wrong on the axis that matters** — measured after the build the same day, catering is **$20.10M of $86.38M `net_sales_l365` (23.3%)** across 22,698 customers, so an inclusive tier list is a catering-account list at the top. **Closed the same day (Asana 1218627569715877, steward instruction):** `catering_orders_l30/l90/l365` + `catering_net_sales_l30/l90/l365` added to `sales_ops.customer_attribute` (config patched via the transfer API, manual run `scheduled_query_6ae81840…` 12:42 MT, 13 s / 3.37 GiB, 1,336,338 rows, nesting and ≤-parent checks all zero), and `orders_l365` / `net_sales_l365` / `catering_orders_l365` / `catering_net_sales_l365` exposed on `claude.order_customer` (view redeployed, 74 columns, grain verified 29,289 = 29,289 on 2026-09-16). Also: anchoring the window on `max(business_date)` at 09:35 makes the last day of the window the partial intraday partition; the as-of guard in (1) above (`least(yesterday, max(business_date))`) is the fix. **(b) Channel rebuilt from `destination` a fifth way in two days** (09:35, 2.23 GiB): `'Third party' = destination in ('DoorDash','UberEats','GrubHub','Postmates') or order_source = 'Checkmate'` (still double-listing Checkmate), `'CZ Delivery' = destination = 'CZ Delivery'`, `'Digital pickup' = destination in ('Online Takeout','Good Life Lane')`, else `'In store'` — so Drive Thru, Fundraiser and any digital destination not in that two-value list fall into "In store". The sub-channel *question* is legitimate and `revenue_category` alone cannot answer it (it does not split Digital into pickup vs delivery), which is why it keeps getting rebuilt. The canonical composition is already in this file: `revenue_category` for the rollup, `destination = 'CZ Delivery'` for first-party delivery (§6), and pickup = Digital minus CZ Delivery — recipe added under "Recipes" below so there is one text to copy instead of five. Added to the open channel task rather than a new one. **(c) Routing unchanged, session five:** both queries on `sales_ops.*` direct, `claude.*` unread, stores 1111 / 999 not excluded (1217879450574426). What he did right, and the recipe keeps: `order_sequence` joined on `brink_order_id` **and** `business_date`, `customer_type = 'person'`, `mapped_cust_id is not null`, `avg(net_sales)` as the check-size measure, and the %-of-orders-over-$60 share as a group-vs-group comparison.
   >
   > **Days 18-21 (Thu 09-18 -> Mon 09-21 09:30): one analyst session, three recurrences of open items, zero wall reads, and the pulse stall enters its seventh day.** Volumes: `wbeaman@` **18 MCP / 14.8 GiB (Thu 15:36-16:15)** and nobody else on the connector Fri-Sun; zero `error_result` for any non-steward. Steward: 09-18 40 unlabeled / 48.5 GiB + 69 MCP / 21.7 GiB (the `_l365` catering columns and the stall diagnosis, both committed 09-18); Sat/Sun/Mon **16 unlabeled / 0.21 GiB each** = the 05:30 weather-automation health checks and the 07:50-08:01 QuickSight dataset refreshes, identical footprint all three days, nothing to mine. **(a) 🚨 The stall is still live** - see the LIVE section above for the 09-21 job-log evidence; every identity-keyed answer for `business_date >= 09-15` remains wrong, and the analyst session below did not touch identity, so nothing quoted on 09-18 inherits it. **(b) Price index, second session, same tautology, now weekly** (4 queries / 10.2 GiB): `round(item_gross_sales / qty, 2) as upr` -> modal price per `(item_id, item_size, item_modifier)` per `week(monday)` -> quantity-weighted index vs a 2026-01-26 -> 02-16 base, with `coverage` = share of base sales the matched items carry. The construction is clean; the input is still `qty = round(item_gross_sales / price)`, so the index measures Brink's `price` column by construction and cannot see overrides or realized discounts (1218586407526696, evidence added). Adding `item_modifier` to the grain is right and belongs in the item-discovery rule when a source quantity exists. **(c) Comp store, `store_open_date <= '2025-06-14' and store_id <> 50`, in all 14 remaining queries** - the open-date-offset form again, after the 09-14 measurement that no offset reproduces `store_comp_date` (1217879256348882, evidence added). Also `not in (1111, 999)` absent all session; the comp subquery happens to exclude both, which is the only reason no phantom store appeared. **(d) Channel, the fifth shape, six more times** - identical `case` to 09-17 (`'Third party'` double-listing Checkmate, two-value `'Digital pickup'`, everything else `'In store'`), now the analyst's standing definition (1218587040526929, evidence added). **(e) New demand shape worth a name: event-window YoY with phases.** Comp-store, Sunday-dropped, 364-day-aligned TY/LY, bucketed `BASE` (<= 07-08) / `BREAK` (07-09 -> 07-11) / `ACUTE` (-> 08-08) / `RECOV`, by channel and by catering-vs-individual, plus catering check-size distribution (`approx_quantiles` p50/p90/p99, `>= $1,000` and `>= $2,000` order counts and their net share, `least(net_sales, 1000)` winsorised net) and salads-per-1,000-orders by channel in the acute phase. The KB has no recipe for a phased incident window; the pieces exist (`date_dim.holiday`, `is_comp_store`, `revenue_category`, the 364-day gotcha) and were all rebuilt by hand. Not filed as a mart gap - it is a recipe gap, and the one thing to steal is in the gotcha now: **he dropped both Labor Days in aligned-date space (`2026-08-31` and `2026-09-07`) and the Jul 3-5 closure**, which is the correct handling of the floating-holiday rule this file already states. **(f) Composition tie-out used correctly:** `subtotal + total_discount_amount - net_sales` as a zero-check on the catering window, and `count(*)` vs `count(distinct brink_order_id)` as a grain check - both good habits, both from `sales_ops.order_customer` direct (`claude.*` still unread, session six; 1217879450574426). **Steward patterns mined (09-18 unlabeled, 10:17-12:01): `pulse_new` integrity sweep, nothing new to codify.** Row counts by `business_date` on `pulse.orders` vs `pulse_new.orders` (the two queries that found the stall), duplicate-`id` checks on `pulse_new.orders` / `order_customers` / `customers` / `order_discounts` and `pulse.order_items` (`group by id having count(*) > 1`), and an orphan sweep (`pulse_new.orders left join pulse_new.customers` where the customer is missing, split by `status` / `source` / `customer_id is null`). Three `select *` on that anti-join at **8.15 GiB each** (24.5 GiB of the day's 48.5) - the 09-10 `LIMIT`/`select *` lesson (1218397668807774) applies to the steward's console too. The dedup and orphan shapes are already in the `order_customer` dictionary's health-check section; `pulse_new` itself stays undocumented until the steward decides whether the marts move to it.

   > **Day 22 (Mon 09-21 09:30 -> Tue 09-22 09:40): a clean first session from a new account, and the pulse stall breaks.** Volumes: `melspencer@` **5 MCP / 1.05 GiB (Mon 15:20-15:21)**, first appearance of this account, nobody else on the connector; 1 `error_result`, self-inflicted and cancelled (below). Steward: Mon 390 unlabeled / 50.5 GiB, **all `dashboard.*` cache tables** (the cz-dashboard payload set replayed ~35 times during tab development: the `with w as (select * from unnest([struct('yesterday' ...` window-array shape over `sales_day` / `discount_day` / `item_day` / `rev_center_day` / `customer_day` / `braze_send_day`, plus the as-of guard `least(yesterday, max(business_date) across seven cache tables)`) and 21 MCP / 6.9 GiB (Braze effectiveness tab: `braze_send_day` / `braze_program_dim` row and date-span checks, `count(distinct id)` vs `count(*)` send-dedup on the four `braze.*_send` tables for Aug, and an `employee_discount_amount` tie-out of `dashboard.sales_day` against `claude.order_line_discount_detail` for 09-14 -> 09-19). Tue so far: the 05:30 weather health checks and 07:50-08:01 QuickSight refreshes, 16 / 0.19 GiB, identical to every prior morning. Zero raw reads by anyone; the steward's day was dashboard work already covered by the cz-dashboard repo, nothing to codify here. **(a) The stall broke at 07:57 MT today** - see the LIVE section for the merge evidence and the heal schedule (marts 09-23 05:02, `customer_attribute` 09-24 05:00). **(b) `melspencer@` is textbook and is visibly running the item-sales-builder discovery shape**: `lower(item_name) like '%athlete%'` grouped by `item_name, item_size, item_id, rev_center_name, item_type` with `units` and `avg_unit_price`, the three symmetric `ifnull(...) not in ('Discount','Promotion')` markers, 60-day partition filter, then a single-day pull of the two resolved `item_id`s (643649840, 643649878, 2026-09-19) with `line_item_type in ('item','modifier')` - i.e. resolve the name to ids first, then measure, exactly what the 08-27 size rule asks for. All `claude.*`, zero `sales_ops.*`, zero hand-rolled definitions. **(c) The one miss, and it is a skill-text gap, not a user gap:** the freshness check was first generated as a `full outer join ... on false, claude.order_customer oc` with no partition filter (a full-history cross join of both marts), cancelled by the session in the same second at 0 GiB, and re-issued a minute later as two partition-filtered `max(business_date)` statements (0.03 / 0.04 GiB). The skill said "run `max(business_date)` on every table" without giving the shape; it now gives the shape (Gotcha 1 under the `order_lines` stale section). Also `current_date()` without `'America/Denver'` in the 60-day window - harmless at 15:20 MT, wrong after 6pm (standing gotcha). **Steward patterns mined: nothing new** - the window-array `unnest([struct(...)])` TY/LY/WTD frame and the seven-table as-of guard are cz-dashboard conventions and belong to that repo's skill, not this one.

   > **Day 23 (Tue 09-22 09:40 -> Wed 09-23 09:40): the stall heals on schedule, a first-day console account pays 152 GiB to read one order, and the card-hash identity is now persisted in a dataset an analyst created himself.** Volumes: `wbeaman@` **22 MCP SELECT / 10.8 GiB plus 1 `CREATE SCHEMA` and 4 CTAS / 5.4 GiB (Tue 10:32-12:24)**; `mvalle@` **4 console / 151.8 GiB (Tue 15:42-15:50), first appearance in 180 days of `JOBS_BY_PROJECT`**; nobody else. Zero `error_result`, zero raw reads by any non-steward. Steward: 50 MCP / 8.0 GiB Tue (the App User revision already committed in 2ec603f; a `guest_status` prototype on `scratch.customer_attribute_guest_test`, still an uncommitted working copy; `pulse.orders` heal diagnostics 11:37-11:42, authorized), an hourly `dashboard.requests` / `dashboard.tab_registry` poller at :34 (cz-dashboard, ~0.01 GiB a run), and the usual 16 unlabeled morning checks. The DDL/CTAS below were only visible because this review also read non-SELECT jobs (harness gap 1217494247853173). **(a) Heal:** see the 09-23 update in the LIVE section; `customer_attribute` is right from today, not 09-24. **(b) `mvalle@`, single-order lookup with the partition commented out:** `claude.order_lines` x `claude.order_customer` filtered only on `ol.brink_order_id = 1937692932098` (a 2026-09-08 order at Zupas Boise Downtown, checking `revenue_category` / `destination` on a catering order, going by the commented predicates), joined on `brink_order_id` alone. 62.1 + 29.6 + 30.1 + 30.1 GiB. New gotcha written ("Looking up one order"). Console, not the connector, so no skill text reached this session; same access path as `salmquist@` / `mraza@` / `drobins@`. **(c) `wbeaman@`, the 09-10 card-hash bridge is now a set of tables:** the customer_hash gotcha worked on cost (the hash table was read once, 0.98 GiB probe plus one 2.95 GiB CTAS, against 51 reads / 157 GiB on 09-09), but he materialised it by running **`CREATE SCHEMA claude_scratch`** (14-day default expiry) through the connector, so a non-steward account holds dataset-create on the project. Pipeline: `ord_base` (orders 2024-01 -> 2026-08, `not is_catering`, `min(HashKey)` per order) -> `hash_map` (the unratified rule unchanged: a hash maps to at most 2 loyalty ids and one holds >= 80% of its orders) -> `ord_gid` (`'L' || loyalty_id` else stitched `'L' || hash-mapped id` else `'C' || hash`, with `id_kind` scanned / stitched / card_only / none) -> `guest_12mo`, then per-guest 12-month sales, visits, check average, active months and a web-to-app switcher before/after test, all on that key. That is customer counts and frequency quoted on the second person definition (1218397760310471). Three smaller drifts in the same session: **(i)** his 'App user' segment (any iOS/Android order or scan, 2025-09-01 -> 2026-08-31) has the canonical shape but runs on stitched identity and a fixed window; canonical is `is_app_user` on `claude.order_customer`, and the two will not agree because stitching moves card-only POS orders onto members; **(ii)** a channel bucket labels every unscanned POS order **'Guest checkout in-store (no scan)'**. Guest checkout is digital-only (`is_guest_order` is NULL on POS), and this is the "unidentified POS = guest" conflation the steward's 09-22 `guest_status` definition is meant to end; **(iii)** internal accounts excluded by `guest_email like '%@cafezupas.com'` over a four-way email coalesce; the canonical exclusion is `customer_type = 'internal'`, which his `customer_type = 'person'` guard already applies. Routing unchanged: 100% `sales_ops.*`, zero `claude.*`, stores 1111 / 999 in scope (1217879450574426). **Steward patterns mined: nothing new** beyond the heal diagnostics, which are one-off.

   > **Day 24 (Wed 09-23 09:40 -> Thu 09-24 09:40): builds healthy again, one analyst session that is a catalogue of already-documented recurrences, and the LTO scorecard shape is now fully specified by demand.** Volumes: `wbeaman@` **17 MCP SELECT / 57.4 GiB plus 3 CTAS into `claude_scratch` / 3.6 GiB (Wed 12:17-13:57, Thu 09:36)**; nobody else. Zero `error_result` and zero raw reads by any non-steward. Steward: the 2026-09-23 pulse type change (six `COALESCE(INT64, BOOL)` / NUMERIC-into-INT64 rollbacks 10:02-12:10, fixed and committed in 01f8996; no build error since 12:10, and the 09-24 04:00 `cust_map` and 05:00 full-history runs both completed clean), 16 QuickSight refreshes, the 05:30 weather checks and a cz-dashboard cache rebuild at 15:43 (~12 GiB). **(a) LTO launch benchmark (Asana 1218319740414409):** the W6 Fall 26 launch (Athlete Bowl, Pizza Sandwich, 2026-09-16) scored against 21 LTOs back to Feb 2025 on units per store-day (week 1 / days 7-30 / full), orders per 1,000 chain orders, check average against the same-day base, digital share, and new and 98-day-reactivated person share. Two defects: launch dates were **hand-typed** into an `unnest` list instead of read from `sales_ops.items.launch_date`, and the join is on `item_name`, so Chocolate Strawberry Cup and Dubai Cup blend Mini and Regular (see *item_name is not the product key*). **(b) Pruning:** the customer-mix version filtered `order_lines` and `order_customer` with `business_date in (select d from alld)` and billed **28.7 GiB** for about 140 business dates; the documented fix is an inline `unnest([date ...])` list or a `between` bound (see *A partition filter must be a constant*). **(c) Comp base:** "traded in both windows" (`having min(business_date) <= '2025-09-16' and max(business_date) >= '2026-09-22'`), twice; banned in the Comp store row, canonical `is_comp_store = 1` (1217879256348882). **(d) Channel:** digital as `destination in ('Online Takeout', 'Good Life Lane', 'CZ Delivery')`, third party as a destination list plus `order_source = 'Checkmate'`; canonical `revenue_category` (1218587040526929). **(e) Line-grain sales used as order sales:** launch-week sales YoY is `sum(item_net_sales)` over every `order_lines` row, and the Thursday Kids Meals share divides by `sum(item_net_sales)` over all lines, with the three-marker Discount/Promotion exclusion absent and tips, fees and gift-card lines left in the denominator. YoY sales comes from `order_customer.net_sales`; an item share needs the menu-line filter on both numerator and denominator. **(f) Card-hash identity, day 3:** `internal_gids`, `guest_12mo` and `switch_ba` (a web-to-app switcher difference-in-differences) are now built on the stitched key, so app-switch incrementality is being quoted on the second person definition (1218397760310471). Stores 1111 / 999 in scope throughout (store 50 excluded by hand), and routing unchanged at 100% `sales_ops.*` (1217879450574426). **Steward patterns mined: nothing new.**

   > **Day 25 (Thu 09-24 09:40 -> Fri 09-25 10:00): a first-day analyst runs the KB protocol end to end with no defects, and brings a third market definition.** Volumes: `kvincent@` **10 MCP SELECT / 1.79 GiB (Thu 11:20-12:16), first appearance in 180 days of `JOBS_BY_PROJECT`**; nobody else (the `wbeaman@` Kids Meals share queries at Thu 09:35-09:37 were covered in Day 24 (e)). Zero `error_result` across every job type, zero raw reads by any non-steward. Steward: the chained order-mart query ran every scheduled hour including the 05:00 full-history reload (93.2 GiB), `cust_map` 04:00, `loyalty_user` 04:30, `customer_attribute` 05:20, `store_info` 06:00 (its `assert` guards passed), `items` 07:30, and the Braze/QuickSight 08:45-09:00 set, all clean; MCP work was the cz-dashboard Drink card, New stores and VTO tabs (ledger D-027 / D-028 / D-029, eb739ec), authorized. **(a) `kvincent@`, W6 Fall 26 launch (Athlete Bowl 643649840 / 643649878, 2026-09-16) units by market:** discovery grouped by `item_name, item_size, item_id` with `avg_unit_price` over a bounded 30-day window, then `item_id in (...)` for every reporting query; all three Discount/Promotion markers null-safe; `claude.*` only (the views already drop stores 1111 / 999); both `business_date` bounds on the `order_lines` x `order_customer` join; catering off `oc.is_catering`; `last_day(..., week(monday))` for week ending. This is the model session. Two notes, neither a defect: the launch date is typed as `'2026-09-16'` rather than read from `sales_ops.items.launch_date` (one item, and it is right), and **(b) a forecast market CASE** maps store 162 Zupas St. George (Utah) into `'NV +STG'` with the Nevada stores and every other state to a two-letter code. That is the third hand-rolled geography after jelgie's two DMA versions on 2026-08-28, and the first that crosses a state line, so `market = store_state` cannot reproduce it (evidence added to Asana 1218013366621844). **(c) Two `launch_date` definitions now live side by side:** `sales_ops.items` (first day at >= 25% of the stores the item ever reached) and `dashboard.new_item_dim` (first day at >= 20 stores, D-029). Measured 2026-09-25 over 67 non-left-censored items launching since 2025-01-01: they agree on **all 24** VTO entrees (Entree less Kids Meals, what the tab shows), and differ on 14 others, every one a 2023 staple the breadth rule re-dates to a POS re-issue (the four kids components plus Milk, Apple Juice and No Drink to **2026-06-04**, Dr Pepper Regular to 2026-09-24, Barq's to 2025-11-19). `items.launch_date` dates those correctly to 2023. Rule: quote `sales_ops.items.launch_date` for "when did X launch"; the dashboard cache is only right inside its display filter (Asana 1218866618005577). **Steward patterns mined: nothing new.**
