...
| `rev_center_name` | STRING | Revenue center: Sandwiches, Soups, Salads, Bowls, Combos, Modifiers, Kids Meals, Desserts, Bottled Beverages, **Foutain Beverages** (misspelled in source — match as-is), Sides/Misc Items, Non Food/Bev Mis, Box Lunches, Party Trays & Food, Cater Desserts, Cater Beverages, Gift Cards, Discount, **Promotion** (added 2026-07-31). ⚠️ **NULL only on `surcharge` lines** — 2 lines over 2026-05-03 → 2026-06-27. Promotions carried NULL here until the second 2026-07-31 pass; if you're reading a result set built earlier in that day, expect NULL instead of `'Promotion'`. |
| `item_type` | STRING | Reporting rollup: Bowls/Salads/Sandwiches/Soups → `Entree`; Kids Combo → `Kids Meals` (other kids items → `Entree`); beverages → `Beverage`; `discount` lines → `Discount` (2026-07-30); `promotion` lines → `Promotion` (2026-07-31); else the revenue center, else raw description. **Zero NULLs since 2026-07-31.** The description-fallback "long tail" is effectively gone — measured 2026-07-31 over 2026-05-01 → 2026-07-30 (stores 1111/999 excluded): 16 distinct values = 15 known buckets + **3 lines / $585** of stray value (`10% Delivery Fee` surcharge lines with no item-master row). The old long tail was the unnamed-promotion population, eliminated 2026-07-31. |
...
- `item_type` and `rev_center_name` fall back to raw `description` / NULL for a small tail — filter to known buckets for clean rollups. As of 2026-07-31 `item_type` has **no** NULLs and `rev_center_name`'s only NULLs are **surcharge** lines (2 in the 8-week test window).
...
- **The parent rollup fields (`parent_rev_center_name` / `parent_item_grp_name`) leak raw descriptions in three ways** (measured 2026-07-31 over 2026-05-01 → 2026-07-30, stores 1111/999 excluded; 15.8M lines):
  ...
  3. **Case-order defect**: the `'Combos'` → `'Try 2 Combo'` rename evaluates *before* the Discount/Promotion overrides, so a discount/promotion line whose `combo_order_line_item_id` happens to collide with a real combo parent's id gets `parent_rev_center_name = 'Try 2 Combo'` (1 discount line in the window, with NULL `parent_item_grp_name`).
