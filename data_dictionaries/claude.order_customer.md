...
> **`customer_attribute` is as-of *yesterday*, not real-time.** All `lifetime_*`, `first/last_order_date`, `days_since_last_order`, `customer_tenure_days`, `is_app_user`, `app_user_type`, `app_purchase_mode`, `gender`, `birthday` and `age` values come from a daily build. They do **not** include today's orders, and on the current business date they lag. Don't expect `lifetime_order_count` to equal a `count(*)` you compute from this view over full history.
