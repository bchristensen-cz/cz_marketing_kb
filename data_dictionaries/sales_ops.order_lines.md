...
| `item_name` | STRING | **Normalized item group name** — size prefix (REG/LG/HALF/…) stripped, leading `.`/`--` cleaned. Use this for menu-mix. Falls back to `description` when the item isn't in the item master — and those fallback rows keep any size word in the name while `item_size` reads `'Not Sized'` or `'Regular'` rather than the size the name announces (e.g. `Mini Chocolate Chips`, which is a Mini). ⚠️ **Not a product key.** Re-measured 2026-09-11 over the trailing 30 days 2026-08-12 → 2026-09-10 (sellable lines, stores 1111/999 excluded), **142 of 374 names span more than one `item_id`** and those names carry **50.6% of all units**: 49 split by `item_size`, 93 split by something else. Stable against the first measurement 2026-08-27 (140 of 373, 50.1%, 49/91) — this is a property of the catalogue, not of the window. `Chocolate Strawberry Cup` = two ids at $9.00 and $14.00; `Dubai Cup` = $12.00 and $18.00; `Kids Combo` = a $0.00 `Kids` bundle slot (46,653 units) and a $7.26 `Regular` paid combo (39,844) — the paid one is `Regular` because the family has a `Kids` sibling, and a units count on the name double-counts every kids meal. Resolve to `item_id` before measuring, and show the average unit price in the discovery query so a wrong pick is visible. On `discount` lines (2026-08-12) this is the **discount program name** from `brink.brinkDiscounts` (`SessionM Loyalty`, `Online Discount`, …) — group on it for discount-program reporting. **Not trimmed** (measured 2026-09-05): `'Apple Juice '` (643636585) and `'Strawberry Daiquiri Mocktail '` (643644616/7) carry a trailing space, so `= 'Apple Juice'` returns zero rows — resolve with `like` / `trim()`, then filter on `item_id`. |
| `item_size` | STRING | **Closed 9-value domain, no NULLs** (rebuilt 2026-08-27): `Regular`, `Half`, `Large`, `Kids`, `Mini`, `Party`, `Tray`, plus **`Not Sized`** and **`Not Applicable`**. (`Quart` and `Medium` exist in the build's CASE but have no rows.) The build resolves it in four steps: non-`item`/`modifier` lines → `'Not Applicable'`; a parsed size prefix → that size; otherwise, if the item's family has sized SKUs in the item master → `'Regular'`; else `'Not Sized'`. **Size is stripped out of `item_name`** (regex `^(REG\|Mini\|LG\|PRTY\|HALF\|Kids\|LARGE\|Medium\|Tray\|QUART) `, case-sensitive, applied once), so the spoken name and the column disagree: `item_name = 'Mini Chocolate Strawberry Cup'` returns **zero rows**; the product is `item_name = 'Chocolate Strawberry Cup' and item_size = 'Mini'`. **`Regular` now means an actual regular size** — a base size in a family that has other sizes, or a genuine `REG` prefix. It no longer absorbs unsized items or non-product lines, which is what `Not Sized` and `Not Applicable` are for. Distribution re-measured 2026-09-11 over 2026-09-04 → 2026-09-10, stores 1111/999 excluded: `Not Sized` 757,187 (60.8%, 245 names) / `Half` 166,038 (13.3%, 27) / **`Regular` 147,932 (11.9%, 41)** / `Kids` 71,751 (5.8%, 28) / `Large` 55,046 (4.4%, 40) / `Not Applicable` 38,888 (3.1%, 26) / `Mini` 7,013 (0.6%, **2 names — `Chocolate Strawberry Cup` and `Dubai Cup`, still the only ones**) / `Party` 751 (7) / `Tray` 15 (4). Every share is within ~1 point of the 2026-08-20 → 08-26 measurement, so treat this as the settled shape. ⚠️ **Two earlier semantics shipped and were replaced the same day** — NULL-for-unparsed (before 2026-08-27 ~10:53 MT) and `coalesce(..., 'Regular')` which made `Regular` 76.3% of lines (10:53 → 12:35 MT). A query written against either will still run and return a different number. `item_size is null` in particular is now **dead** — it returns zero rows, not the unsized items. **Required as a dimension on any item-specific question** — 49 names / 21.3% of units span more than one size, and a name-only answer blends price points (Mini cup $9.00 vs Regular $14.00 → a $10.25 average that is not a real price). ⚠️ **The full-size Chocolate Strawberry Cup (643640567) is `Regular`, not NULL** — it was NULL-sized before the 2026-08-27 rebuild, so notes predating that are now wrong in the opposite direction. |
