...
| `orders_l365` | INT64 | `customer_attribute` | **New 2026-09-18.** The customer's order count in the 365 days ending `attribute_asof_date` (yesterday), person-only, store 1111/999 excluded, **catering included**. NULL on unidentified / non-person orders, same convention as `lifetime_net_sales`. Use it for spend / frequency tiers instead of re-aggregating this view (Asana 1218627569715877) |
| `net_sales_l365` | FLOAT | `customer_attribute` | **New 2026-09-18.** Net sales over the same window. Ex-catering: `net_sales_l365 - catering_net_sales_l365` |
| `catering_orders_l365` / `catering_net_sales_l365` | INT64 / FLOAT | `customer_attribute` | **New 2026-09-18.** The `is_catering` subset of the two columns above. Catering is **23.3% of chain-wide `net_sales_l365`** ($20.10M of $86.38M on the 2026-09-18 build), so the standard catering exclusion materially changes any top-spender list. The 30/90-day pairs exist on `sales_ops.customer_attribute` only |
| `is_app_user` | BOOL | `customer_attribute` | **New 2026-09-17, revised 2026-09-22.** The canonical App User flag for the customer on this order: non-catering app order or in-store scan in the trailing 12 months, as of the attribute build (catering orders never count, revision 2026-09-23). **Since 2026-09-22 a native-app session alone does not qualify** (it did from 09-18 to 09-21; browsing without buying is not app usage). **NULL, not false, on unidentified and non-person orders** (~60% of orders) — filter `is_app_user is true`, and never read `is not true` as "not an app user" without also requiring `customer_type = 'person'`. Definition and rules: `sales-ops-orders` skill, *"App user" is a canonical customer definition* |
| `app_user_type` | STRING | `customer_attribute` | **New 2026-09-17, revised 2026-09-22.** `purchase_and_session` / `purchase_only`; **NULL** when not an app user *and* when unidentified, so a NULL alone does not distinguish the two. The `session_only` value was retired 2026-09-22 (those customers are no longer app users). Measured on the week ending 2026-09-16 under the old definition: of 47,284 identified person orders, **37,902 (80%)** were placed by `purchase_and_session` customers, 1,736 by `purchase_only`, 290 by `session_only`, 7,356 by non-app-users |
| `app_purchase_mode` | STRING | `customer_attribute` | **New 2026-09-22.** How the app user on this order buys, over the trailing 12 months as of the attribute build: `app_orders_and_scans` / `app_orders_only` / `scans_only`; **NULL** when not an app user and when unidentified. Mutually exclusive; splits the two halves of the purchase test. Value analysis to 2026-09-14 (`claude/app-segments-value-2026-09-22.md`): the `app_orders_and_scans` group is 23% of app users and 44% of app-user restaurant sales at 9.5 orders / $220 a year; `app_orders_only` and `scans_only` are near twins at ~4 orders / $92-97 a year. It describes the customer, not the order: a scan by an `app_orders_and_scans` customer still carries that value. |
| `guest_status` | STRING | `customer_attribute` | **New 2026-09-22.** Customer-level guest state of the customer on this order, as of the attribute build: `guest` (never placed an authenticated order), `converted_guest` (first order was a guest checkout, has since authenticated), `account` (authenticated first; an occasional later guest order does not demote them). **NULL when unidentified / non-person.** Describes the customer, not the order: `is_guest_order` is the order flag. Lifetime, so most `guest` customers are 2023 one-timers; pair with `first_order_date >= '2026-07-01'` for the relaunch cohort. Supporting columns (`first_order_was_guest`, `last_order_was_guest`, `guest_orders_l365`) are `sales_ops`-only. |
...
> **`customer_attribute` is as-of *yesterday*, not real-time.** All `lifetime_*`, `first/last_order_date`, `days_since_last_order`, `customer_tenure_days`, `is_app_user`, `app_user_type`, `app_purchase_mode`, `guest_status`, `gender`, `birthday` and `age` values come from a daily build. They do **not** include today's orders, and on the current business date they lag. Don't expect `lifetime_order_count` to equal a `count(*)` you compute from this view over full history.
