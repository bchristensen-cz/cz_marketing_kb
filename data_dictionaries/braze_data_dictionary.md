...
> - **Column names analysts guess wrong (each verified against `INFORMATION_SCHEMA` 2026-08-31 after live failures):** `braze.users` has **`created_at` only — no `updated_at`** (freshness for the profile export comes from `braze.load_watermark`, not a row column); `pushnotification_tokenstatechange`'s state column is **`push_token_state_change_type`** (not `token_state`); the experiment control flag on `canvas_experimentstep_splitentry` is **`in_control_group`** (not `in_control`).
