...
| `item_name` | STRING | **Normalized item group name** — size prefix (REG/LG/HALF/…) stripped, leading `.`/`--` cleaned. Use this for menu-mix. Falls back to `description` when the item isn't in the item master — and those fallback rows keep any size word in the name with a NULL `item_size` (e.g. `Mini Chocolate Chips`). ⚠️ **Not a product key.** Measured 2026-08-27 over the trailing 30 days (sellable lines, stores 1111/999 excluded), **140 of 373 names span more than one `item_id`** and those names carry **50.1% of all units**: 49 split by `item_size`, 91 split by something else. `Chocolate Strawberry Cup` = two ids at $9.00 and $14.00; `Dubai Cup` = $12.00 and $18.00; `Kids Combo` = a $0.00 bundle slot and a $7.27 paid combo. Resolve to `item_id` before measuring, and show the average unit price in the discovery query so a wrong pick is visible. On `discount` lines (2026-08-12) this is the **discount program name** from `brink.brinkDiscounts` (`SessionM Loyalty`, `Online Discount`, …) — group on it for discount-program reporting. |
| `item_size` | STRING | Parsed size prefix: `Half`, `Large`, `Regular`, `Kids`, `Mini`, `Party`, `Tray`, `Quart`, `Medium` or NULL. **Stripped out of `item_name` by the build** (regex `^(REG\|Mini\|LG\|PRTY\|HALF\|Kids\|LARGE\|Medium\|Tray\|QUART) `, case-sensitive, applied once), so the spoken name and the column disagree: `item_name = 'Mini Chocolate Strawberry Cup'` returns **zero rows**; the product is `item_name = 'Chocolate Strawberry Cup' and item_size = 'Mini'`. ⚠️ **NULL is not `Regular`.** NULL means no prefix was parsed and covers both genuinely unsized items *and* the default size of a sized family — the full-size Chocolate Strawberry Cup is NULL, so `item_size = 'Regular'` on it returns zero even though `Regular` is populated on 22 other names. Use `coalesce(item_size, '(no size)')` in every breakout. **Required as a dimension on any item-specific question** — 49 names / 21.0% of units span more than one size, and a name-only answer blends price points (Mini cup $9.00 vs NULL $14.00 → a $10.23 average that is not a real price). Line counts on sellable lines, 2026-07-29 → 2026-08-26, stores 1111/999 excluded (measured 2026-08-27): NULL 3,828,070 (72.7%) / `Half` 683,604 / `Kids` 296,243 / `Large` 234,690 / `Regular` 190,934 / `Mini` 30,892 (only `Chocolate Strawberry Cup` and `Dubai Cup`) / `Party` 4,513 / `Tray` 130; `Quart` and `Medium` **zero rows in the window** despite being in the build's CASE. |
