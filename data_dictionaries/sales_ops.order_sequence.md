...
| `mapped_cust_id` | INTEGER | Canonical customer key. Never NULL in this table. **Redefined 2026-09-09 18:02 MT** (deployed in the scheduled query then; the repo caught up 2026-09-23) as the first non-null of, in order (all flags from `order_customer`; `m` = `cust_map` joined on the ORDER `email`, `ma` = `cust_map` joined on the account's `acct_email`, `ms` = `cust_map` joined on `sm_email`; every branch reads `cust_map.acct_id`): (1) `coalesce(pulse_customer_id, sm_external_user_id)` when `is_catering` — catering accounts are their own identity population; (2) `m.acct_id` when `is_sys_order_email = 0 and email is not null` — the person by the email typed on the order, whether guest, authenticated or on a system account; (3) `ma.acct_id` when `is_sys_acct_email = 0 and acct_email is not null` — order email is a system address or missing, so the person by the signed-in account's own email (kiosk / operator loyalty scans); (4) `coalesce(ms.acct_id, sm_external_user_id)` when `sm_email is not null` — no usable Pulse email anywhere, SessionM identity; (5) `pulse_customer_id` when both order and account emails are system addresses — pure system identities (kiosk terminal, checkmate). Orders resolving to NULL (third-party aggregator orders with a `guest.doordash.com` / `itsacheckmate.com` order email and no loyalty scan, most notably) have **no row here**. The 2026-09-08 seven-branch version (which read `max_acct_id` / `final_acct_id` and keyed a guest branch on `is_guest_order`) ran for 25 hours only, 2026-09-08 17:02 to 2026-09-09 17:02. Steward intent per branch is in the inline comments of `sql/sales_ops.order_marts.sql`; the measured shift vs the pre-09-08 definition is not written up. |
