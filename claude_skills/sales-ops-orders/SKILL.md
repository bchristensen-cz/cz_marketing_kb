...
**Annual spend tiers — "who are our $1,000+/year customers?" (added 2026-09-18):** do not re-aggregate `order_customer` for this. `sales_ops.customer_attribute.net_sales_l365` / `orders_l365` are the trailing-365-day spend and order count per person customer, rebuilt daily, store 1111 excluded. State the two things the dimension fixes for you: the window is **365 days to yesterday** (not to `max(business_date)`, whose last day is the partial intraday partition), and **catering is included** with no in-window net-out (only `lifetime_catering_order_count` exists; catering and individual customers are separate identity populations, so the residual is small, but say so). A standard user without `sales_ops` access joins the folded `lifetime_*` columns on `claude.order_customer` instead and does not get the `_l365` pair (gap logged).
```sql
select
case
	when ca.net_sales_l365 >= 5000 then 'e. $5,000+'
	when ca.net_sales_l365 >= 3000 then 'd. $3,000-4,999'
	when ca.net_sales_l365 >= 2000 then 'c. $2,000-2,999'
	when ca.net_sales_l365 >= 1000 then 'b. $1,000-1,999'
	else 'a. under $1,000'
	end as tier
, count(*) as customers
, round(sum(ca.net_sales_l365)) as net_sales_l365
, round(avg(ca.net_sales_l365), 2) as avg_spend
, round(avg(ca.orders_l365), 1) as avg_orders
from `marketing-data-442316`.sales_ops.customer_attribute ca
where 1=1
and ca.orders_l365 > 0
group by 1
order by 1
```
